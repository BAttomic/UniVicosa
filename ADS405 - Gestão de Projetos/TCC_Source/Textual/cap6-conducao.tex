...
\begin{itemize}
    ...
    \item Exposição ao risco (Probabilidade \(\times\) Impacto)
...
\begin{itemize}
    \item \textit{Mitigação:} Fila de pré-venda por ordem de chegada (FIFO) sobre o MongoDB para eventos com vendas agendadas; fila distribuída (Redis) como evolução para escala massiva
    \item \textit{Mitigação:} Aplicação \textit{stateless} permitindo escala horizontal (mais instâncias) em produção quando necessário
    \item \textit{Mitigação:} Utilização de CDN para distribuir carga de assets estáticos (produção)
    \item \textit{Monitoramento:} Testes de carga progressivos a cada 3 sprints
\end{itemize}

\textbf{R002 - Falhas na Integração de Pagamentos:}
\begin{itemize}
    \item \textit{Mitigação:} No protótipo o pagamento é simulado, isolando o risco; em produção, integração ao gateway Asaas
    \item \textit{Mitigação:} Tratamento de erro e reprocessamento idempotente do pedido (aprovação atômica \textit{pending}$\rightarrow$\textit{paid})
    \item \textit{Mitigação:} Testes em ambiente sandbox do gateway antes de produção
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{Mudança Maior:} Impacto \(\geq\) 5\%, requer aprovação do orientador acadêmico
    \end{itemize}
...
\begin{itemize}
    ...
    \item Testes unitários escritos e passando (cobertura \(\geq\) 80\%)
...
\begin{itemize}
    \item Verificação de tipos (TypeScript estrito) e ESLint na esteira de CI (GitHub Actions)
    \item A esteira falha quando há erros de tipo ou de \textit{lint}, impedindo a integração
    \item Ferramentas dedicadas de análise (ex.: SonarQube) ficam como evolução futura
\end{itemize}
...
\textbf{Gateway de Pagamento (Asaas, para produção):} Avaliação baseada em:
\begin{itemize}
...
\begin{itemize}
    ...
    \item Componentes de software (modulos)
