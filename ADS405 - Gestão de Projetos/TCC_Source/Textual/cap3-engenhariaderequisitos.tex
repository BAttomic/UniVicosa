...
\begin{itemize}
    ...
    \item Perfil global de administrador, com acesso de gestão a todos os eventos e usuários da plataforma
...
\begin{itemize}
    \item Seleção de tipo de ingresso, lote ativo e quantidade
    \item Captura dos dados do titular (nome e CPF) e da forma de pagamento simulada (PIX, cartão ou boleto)
    \item \textbf{Pagamento simulado}: o pedido é registrado como pendente e aprovado automaticamente após curto intervalo, emitindo os ingressos, sem integração a \textit{gateway} financeiro real (decisão de escopo do protótipo acadêmico)
    \item Histórico de pedidos com busca e filtros por status e período
\end{itemize}
...
\begin{itemize}
    \item Conformidade com os padrões PCI DSS prevista para a futura integração a um \textit{gateway} de pagamento real; no protótipo o pagamento é simulado e não há tráfego de dados de cartão~\cite{PCIDSS:2022}
