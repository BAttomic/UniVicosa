...
Esta fase estabelece as bases para todo o projeto, incluindo analise completa de requisitos, definicao da arquitetura de software com Next.js e MongoDB, e planejamento detalhado de recursos, cronograma e orcamento.
...
\textbf{1.1.2 Definicao de Arquitetura:} Projeto da arquitetura full-stack com Next.js e MongoDB, incluindo diagramas de componentes, definicao de tecnologias (linguagens, frameworks, banco de dados) e padroes de comunicacao entre modulos. Entrega: Documento de Arquitetura de Software.
...
Estabelecimento do ambiente de execução e configuração das camadas de segurança da aplicação (cabeçalhos de segurança, proteção de rotas e \textit{rate limiting}). A conformidade com PCI DSS~\cite{PCIDSS:2022} aplica-se à futura integração de pagamento real.

\textbf{1.2.1 Configuração de Ambiente:} Provisionamento em servidor próprio com Easypanel (PaaS baseado em Docker) e MongoDB. A aplicação é \textit{stateless}, permitindo escala horizontal (múltiplas instâncias) em produção quando a demanda exigir. Entrega: Ambientes de desenvolvimento e produção operacionais.
...
Desenvolvimento do backend e APIs do Next.js com modulos de dominio, garantindo separacao de responsabilidades, escalabilidade horizontal e manutenibilidade.
...
\textbf{1.3.3 API de Processamento de Pagamentos:} Desenvolvimento do carrinho de múltiplos tipos de ingresso, do pagamento (simulado no protótipo, com o gateway Asaas previsto para produção), do controle da quantidade vendida por lote e da geração de comprovantes. Entrega: API de pedidos testada e documentada.
...
\textbf{1.5.1 Gateway de Pagamento:} Integração prevista para produção com o gateway \textbf{Asaas} (PIX e cartão), incluindo \textit{webhooks} de confirmação. No protótipo o pagamento é simulado, com aprovação automática do pedido. Entrega: Pagamento simulado funcional; integração Asaas especificada.
...
\textbf{1.6.1 Testes Unitários:} Desenvolvimento e execução de testes unitários automatizados (Vitest) cobrindo as regras de negócio críticas — uso único do ingresso, cálculo da taxa e fluxos de pedido — com MongoDB em memória. Entrega: Suite de testes unitários aprovada.

\textbf{1.6.2 Testes de Integracao:} Execucao de testes automatizados validando a comunicacao entre modulos internos, integracao com APIs externas e fluxos completos de funcionalidades. Entrega: Suite de testes integracao aprovada.

\textbf{1.6.3 Testes de Carga (produção futura):} Testes de carga e estresse para validar a escala horizontal e identificar gargalos. Previstos para a fase de produção, fora do escopo do protótipo acadêmico. Entrega: Relatório testes carga (futuro).

\textbf{1.6.4 Testes de Segurança:} Verificação estática (TypeScript estrito e ESLint) e revisão de aderência ao OWASP Top 10~\cite{OWASP:2021}. Análise dinâmica (DAST) e teste de penetração por especialista externo ficam previstos para a fase de produção. Entrega: Revisão de segurança da aplicação.
...
\textbf{1.7.1 Deploy em Produção:} Implantação pelo Easypanel a partir da imagem Docker (saída \textit{standalone} do Next.js), com variáveis de ambiente injetadas em \textit{runtime} e atualização automática a cada \textit{push}. Entrega: Sistema em produção operacional.
...
\label{tab:tarefas}
\footnotesize
\begin{tabular}{|p{0.9cm}|p{6.2cm}|p{5.7cm}|c|}
\hline
...
1.2.1 & Projetar arquitetura Next.js + MongoDB & Definir camadas, modulos e modelagem do banco & Não \\ \hline
...
1.3.2 & Configurar escala horizontal & Manter a aplicação \textit{stateless} para múltiplas instâncias quando necessário & Não \\ \hline
...
2.3.3 & Integrar gateway de pagamento & Pagamento simulado no protótipo; integração Asaas prevista para produção & Não \\ \hline
...
4.3.1 & Implementar fila de pré-venda & Fila FIFO sobre o MongoDB (sem Redis) & Não \\ \hline
...
4.3.4 & Testar a fila de pré-venda & Validar ordem de chegada (FIFO) e posição na fila & Sim \\ \hline
...
5.1.5 & Cobrir regras críticas & Testar uso único do ingresso, taxa e fluxos de pedido & Sim \\ \hline
...
5.4.1 & Verificação estática & TypeScript em modo estrito e ESLint & Não \\ \hline
5.4.2 & Revisar OWASP Top 10 & Revisar aderência às boas práticas de segurança & Não \\ \hline
5.4.3 & Teste de penetração (produção futura) & Previsto para a fase de produção & Não \\ \hline
5.4.4 & Remediar vulnerabilidades & Corrigir todas as vulnerabilidades críticas & Não \\ \hline
5.4.5 & Revisão de segurança da aplicação & Validar cabeçalhos, \textit{rate limiting} e LGPD & Sim \\ \hline
6.1.1 & Preparar ambiente de produção & Validar configurações finais de infra & Não \\ \hline
6.1.2 & Executar deploy (Easypanel) & Implantar a imagem Docker a cada \textit{push} & Não \\ \hline
...
1.3.2 & Configurar escala horizontal & 3 dias & 1.3.1 & Término-Início (TI) \\ \hline
...
4.3.1 & Implementar fila de pré-venda & 3 dias & 1.3.1 & Término-Início (TI) \\ \hline
...
5.4.1 & Verificação estática & 3 dias & 5.1.5 & Término-Início (TI) \\ \hline
5.4.2 & Revisar OWASP Top 10 & 3 dias & 5.2.4 & Término-Início (TI) \\ \hline
5.4.3 & Teste de penetração (futuro) & 1 semana & 5.4.2 & Término-Início (TI) \\ \hline
...
6.1.2 & Deploy (Easypanel) & 1 dia & 6.1.1 & Término-Início (TI) \\ \hline
