...
\textbf{3. Sistema de Comercialização e Pagamentos (Simulado)}

Fluxo completo de compra desde a seleção de ingressos até a emissão, com captura dos dados do titular (nome e CPF). Em conformidade com o caráter acadêmico do trabalho, \textbf{o pagamento é simulado}: o pedido é registrado com a taxa de serviço de 5\% e aprovado automaticamente após um curto intervalo, emitindo os ingressos, sem integração a um \textit{gateway} financeiro real. Resultados esperados:

\begin{itemize}
    \item Tempo médio de finalização de compra inferior a 3 minutos
    \item Captura de PIX, cartão ou boleto como forma de pagamento simulada
    \item Histórico de pedidos com busca e filtros por status e por período
    \item Detalhamento da taxa de serviço de 5\% e do repasse integral ao organizador
    \item Arquitetura preparada para a futura integração a um \textit{gateway} certificado PCI DSS
\end{itemize}
...
\textbf{6. Painel de Analytics da Plataforma}
...
\begin{itemize}
    ...
    \item Métricas comparativas com o período anterior — crescimento de receita, de pedidos e de ingressos
    \item Tendência de faturamento por dia/mês e ranking de eventos por receita
    \item Cortesias contabilizadas à parte, sem impacto sobre a receita
\end{itemize}

\subsection{Arquitetura e Infraestrutura}

A plataforma será construída sobre uma arquitetura full-stack baseada em Next.js e MongoDB, implantada em infraestrutura de nuvem com as seguintes caracteristicas:

\textbf{Arquitetura Full-Stack:}
\begin{itemize}
    \item Aplicacao Next.js com renderizacao no servidor e rotas de API integradas
    \item Modulos de dominio para identidade, eventos, equipe por evento, ingressos, pedidos, pagamentos, monetizacao, notificacoes, check-in e privacidade (LGPD)
    \item Banco de dados MongoDB com colecoes e indices otimizados
    \item Containerizacao com Docker quando necessario, com deploy simplificado
\end{itemize}

\textbf{Infraestrutura Cloud:}
\begin{itemize}
    \item Ambientes segregados (desenvolvimento, homologação, produção)
    \item Escala horizontal da aplicação \textit{stateless} (mais instâncias) prevista para produção
...
\begin{itemize}
    \item Arquitetura preparada para certificação PCI DSS na futura integração a um \textit{gateway} real (no protótipo, o pagamento é simulado)
...
\begin{itemize}
    \item Código modular e tipado (TypeScript em modo estrito), facilitando a manutenção
...
\begin{itemize}
    ...
    \item Relatórios automatizados de vendas, público e receita, com detalhamento da taxa de serviço e do repasse ao organizador
    \item Repasse integral do preço do ingresso, com taxa de serviço de 5\% (uma das menores do mercado) somada ao comprador
...
\begin{itemize}
    \item Consolidacao de conceitos de arquitetura full-stack, Next.js, MongoDB e cloud computing
...
Disponibilidade do sistema & > 99,9\% & APM e logs & Contínuo \\ \hline
Tempo resposta P95 APIs & < 500ms & APM & Contínuo \\ \hline
Tempo carregamento páginas & < 2s & Lighthouse & Semanal \\ \hline
Regras críticas com testes & Cobertas & Vitest & Sprint \\ \hline
Verificação estática sem erros & Sim & TypeScript/ESLint & Sprint \\ \hline
...
\begin{itemize}
    ...
    \item Manuais de usuário para cada perfil (comprador, organizador, operador e administrador)
    \item Diagrama de arquitetura dos módulos de domínio e do fluxo de dados
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Fase 3 (6-12 meses):} Aplicativos móveis nativos iOS/Android, analytics avançado e integração com redes sociais
