...
O sistema deve ser escalável horizontalmente, mantendo a aplicação \textit{stateless} para operar em múltiplas instâncias. A meta de suportar picos de até 10.000 usuários simultâneos é um objetivo de produção; no protótipo, a escalabilidade é validada arquiteturalmente.

\textbf{RNF002 - Segurança:}
A plataforma deve aplicar boas práticas de segurança (cabeçalhos, proteção de rotas, \textit{rate limiting}, \textit{hashing} de senhas e LGPD). Os padrões PCI DSS aplicam-se à futura integração de pagamento real (Asaas); no protótipo o pagamento é simulado, sem tráfego de dados de cartão.
...
\begin{itemize}
    ...
    \item Construir fluxo de compra otimizado com carrinho de múltiplos ingressos e pagamento simulado (gateway Asaas previsto para produção)
...
\begin{itemize}
    ...
    \item Fila de pré-venda por ordem de chegada (FIFO) para eventos com vendas agendadas
    \item Processo de compra completo com confirmação automática por e-mail
    \item Pagamento simulado, com a integração ao gateway Asaas (sob normas PCI DSS) prevista para produção
...
\begin{itemize}
    ...
    \item Operadores de portaria e controle de acesso (designados por evento)
    \item Administradores da plataforma (governança de eventos e usuários)
\end{itemize}
...
\begin{itemize}
    \item Servidor próprio com Easypanel (PaaS baseado em Docker) e MongoDB; aplicação \textit{stateless} para escala horizontal
    \item CDN para distribuição global de conteúdo (produção)
    \item Gateway de pagamento Asaas para a fase de produção (pagamento simulado no protótipo)
...
\begin{itemize}
    \item Orcamento total estimado: R\$ 118.000,00
    \item Recursos humanos: R\$ 97.300,00 (aprox. 82\%)
    \item Infraestrutura e tecnologia: R\$ 17.500,00 (aprox. 15\%)
    \item Contingencia e imprevistos: R\$ 3.200,00 (aprox. 3\%)
\end{itemize}
...
\begin{itemize}
    ...
    \item Conformidade PCI DSS prevista para a futura integração de pagamento (Asaas); no protótipo não há tráfego de dados de cartão
